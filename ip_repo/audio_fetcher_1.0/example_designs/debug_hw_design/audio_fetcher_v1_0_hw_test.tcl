# Runtime Tcl commands to interact with - audio_fetcher_v1_0

# Sourcing design address info tcl
set bd_path [get_property DIRECTORY [current_project]]/[current_project].srcs/[current_fileset]/bd
source ${bd_path}/audio_fetcher_v1_0_include.tcl
...
# Test: M_AXI_AUDIO_OUT
set wdata_m_axi_audio_out [get_init_data 0]
create_hw_axi_txn w_m_axi_audio_out_addr [get_hw_axis $jtag_axi_master] -type write -address $axi_gpio_out_addr -data ${wdata_m_axi_audio_out}
create_hw_axi_txn r_m_axi_audio_out_addr [get_hw_axis $jtag_axi_master] -type read -address $axi_gpio_in_addr 
# Initiate transactions
run_hw_axi r_m_axi_audio_out_addr
run_hw_axi w_m_axi_audio_out_addr
run_hw_axi r_m_axi_audio_out_addr
set rdata_tmp [get_property DATA [get_hw_axi_txn r_m_axi_audio_out_addr]]
set DE [ get_done_and_error_bit $rdata_tmp 2 0 ]
# Compare read data
if { $DE == 01 } {
	puts "Data comparison test pass for - M_AXI_AUDIO_OUT"
} else {
	puts "Data comparison test fail for - M_AXI_AUDIO_OUT, rdata-$rdata_tmp expected-01 actual-$DE"
	inc ec
}

# Test: M_AXI_DMA
set wdata_m_axi_dma [get_init_data 1]
create_hw_axi_txn w_m_axi_dma_addr [get_hw_axis $jtag_axi_master] -type write -address $axi_gpio_out_addr -data ${wdata_m_axi_dma}
create_hw_axi_txn r_m_axi_dma_addr [get_hw_axis $jtag_axi_master] -type read -address $axi_gpio_in_addr 
# Initiate transactions
run_hw_axi r_m_axi_dma_addr
run_hw_axi w_m_axi_dma_addr
run_hw_axi r_m_axi_dma_addr
set rdata_tmp [get_property DATA [get_hw_axi_txn r_m_axi_dma_addr]]
set DE [ get_done_and_error_bit $rdata_tmp 2 1 ]
# Compare read data
if { $DE == 01 } {
	puts "Data comparison test pass for - M_AXI_DMA"
} else {
	puts "Data comparison test fail for - M_AXI_DMA, rdata-$rdata_tmp expected-01 actual-$DE"
	inc ec
}
