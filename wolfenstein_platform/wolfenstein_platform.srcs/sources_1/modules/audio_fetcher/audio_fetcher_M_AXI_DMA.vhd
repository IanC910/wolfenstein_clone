...
entity audio_fetcher_M_AXI_DMA is
    generic (
...
architecture implementation of audio_fetcher_M_AXI_DMA is
...
begin
    ...
    process(M_AXI_ACLK) begin
        if(rising_edge(M_AXI_ACLK)) then
            if(M_AXI_ARESETN = '0') then
                axi_rdata_ff    <= std_logic_vector(to_unsigned(11111111, axi_rdata_ff'length));
                axi_rid_ff      <= (others => '0');
                error_ff        <= '0';
            else
