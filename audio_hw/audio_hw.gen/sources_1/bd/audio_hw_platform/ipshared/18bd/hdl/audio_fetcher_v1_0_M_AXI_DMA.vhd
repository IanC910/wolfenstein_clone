...
begin
    ...
    read_ready      <= '1' when (curr_state_ff = IDLE) else '0';

    M_AXI_ARVALID   <= '1' when (curr_state_ff = ARVALID) else '0';
    ...
    M_AXI_RREADY    <= '1' when (curr_state_ff = RREADY) else '0';

    return_valid    <= '1' when (curr_state_ff = RETURNING) else '0';
    return_data     <= axi_rdata_ff;
    return_id       <= axi_rid_ff;

    return_error <= error_ff;
    ...
    process(M_AXI_ACLK) begin
        if (rising_edge (M_AXI_ACLK)) then
            if (M_AXI_ARESETN = '0') then
                ...
                if(curr_state_ff = IDLE and next_state = ARVALID) then
                    axi_araddr_ff   <= read_addr;
                    axi_arid_ff     <= read_id;
                end if;
            end if;
        end if;
